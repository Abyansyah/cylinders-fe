...
import { getSupplierById, updateSupplierPriceList } from '@/services/supplierService';
...
export default function SupplierPriceListForm() {
  const params = useParams();
  const router = useRouter();
  const supplierId = Number(params.id as string);

  const { data: supplier, isLoading: isLoadingsupplier } = useSWR(supplierId ? `/suppliers/${supplierId}` : null, () => getSupplierById(supplierId));
  ...
  useEffect(() => {
    if (supplier?.pricelists) {
      const initialData = supplier.pricelists.map((item) => ({
  ...
  const onSubmit = async (data: PriceListFormValues) => {
    const payload: any[] = data.pricelists.map((item) => ({
      product_id: Number(item.product_id),
      buy_price: item.buy_price ? parseFloat(item.buy_price) : null,
    }));

    try {
      await updateSupplierPriceList(supplierId, payload);
  ...
  if (isLoadingsupplier || isLoadingProducts) return <div>Loading...</div>;
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Atur harga sewa dan beli khusus untuk {supplier?.name}</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button type="button" variant="outline" onClick={() => append({ product_id: '', buy_price: '' })}>
