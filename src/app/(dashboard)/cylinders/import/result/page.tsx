...
import { IMPORT_RESULT_STORAGE_KEY, IMPORT_RESULT_EXPIRY_HOURS } from '@/constants/cylinder-import';
import type { ImportResult } from '@/types/cylinder-import';
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center space-x-2">
        <Button variant="ghost" size="sm" onClick={() => router.push('/cylinders/import')}>
          <ArrowLeft className="h-4 w-4 mr-2" />
          Kembali
        </Button>
      </div>
      <div>
        <h2 className="text-3xl font-bold tracking-tight">Hasil Import Tabung</h2>
        <p className="text-muted-foreground">Detail hasil proses import data tabung</p>
      </div>
                ...
                <TableBody>
                  {response.failed_rows.map((row, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>{row.data.barcode_id}</TableCell>
